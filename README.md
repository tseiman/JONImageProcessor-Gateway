...
The top status row shows the gateway Git hash from `src/version-info.json`. If that commit was exactly on a release tag when `npm run version:write` was executed, the release tag is shown as well. The same row also shows the JONImageProcessor version from IPC `system.version` or `version` when the current processor reports it. FPS, CPU, memory, and the single/dual-screen indicator are read from IPC `benchmark` when benchmark collection is enabled in JONImageProcessor with `--benchmark` or `diagnostics.benchmark=true`. The running service never calls `git`. As an emergency override, set `JON_GATEWAY_GIT_HASH` and optionally `JON_GATEWAY_RELEASE_TAG` in `/opt/JONImageProcessor-Gateway/etc/token.env`.
