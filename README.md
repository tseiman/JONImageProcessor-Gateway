...
`pause.source` selects what JONImageProcessor renders while the primary camera is paused, connecting, or disconnected and `pause.enabled=true`. `image` uses the configured pause asset from `pause.image`; `camera` uses the secondary camera pipeline reported by IPC as `secondaryCamera.pipeline`, for example a GStreamer appsink pipeline fed by AirPlay/uxplay. The pipeline itself is startup configuration in JONImageProcessor and is read-only through the gateway. `pause.preserveAspectRatio=true` letterboxes secondary camera pause frames into the output canvas instead of stretching them.

For existing deployments, add `pause.source` and `pause.preserveAspectRatio` to `api.commands.get.keys`, add `secondaryCamera.pipeline` to `api.commands.get.keys`, and add `"pause.source": { "type": "string", "enum": ["image", "camera"] }` plus `"pause.preserveAspectRatio": { "type": "boolean" }` to `api.commands.set.items` in `/opt/JONImageProcessor-Gateway/etc/gateway.config.json`.
...
For TTF fonts, `pause.font` is set to the safe base name without `.ttf`. Downloads accept either the safe base name or the `.ttf` file name and reject traversal paths. `pause.fontDirectory` is queried over IPC and displayed by the WebUI, but it is not writable through the gateway. `pause.fontAlign` accepts `left`, `center`, or `right`. `pause.source` accepts `image` or `camera`; `pause.preserveAspectRatio` is boolean; `secondaryCamera.pipeline` is exposed as a read-only IPC value.
